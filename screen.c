...
void screen_gets(char *str) {
    ...
    if (fgets(str, COLUMNS, stdin)) *(str + strlen(str) - 1) = 0; /* Replaces newline character with '\0' */
}
...
void screen_area_scroll_up(Area *area) {
    for (area->cursor = ACCESS(__data, area->x, area->y); area->cursor < ACCESS(__data, area->x + area->width, area->y + area->height - 2); area->cursor += COLUMNS) {
...
void screen_utils_replaces_special_chars(char *str) {
    ...
    while ((pch = strpbrk(str, "�?É�?ÓÚÑáéíóúñ")))
        memcpy(pch, "??", 2);
}
